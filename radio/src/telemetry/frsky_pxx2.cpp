...
static void processSpectrumAnalyserFrame(uint8_t module, const uint8_t * frame)
{
  ...
  if (x < LCD_W) {
    ...
    if (reusableBuffer.spectrumAnalyser.bars[x] > reusableBuffer.spectrumAnalyser.peak[x]) {
      reusableBuffer.spectrumAnalyser.peak[x] = reusableBuffer.spectrumAnalyser.bars[x];
    }
#endif
  }
}
