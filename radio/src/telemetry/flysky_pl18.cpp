...
FlyskyPl18Sensor defaultPl18Sensor = {0, 0, "UNKNOWN", UNIT_RAW, 0, 0, 2, false};

const FlyskyPl18Sensor Pl18Sensor[]=
{
...
const FlyskyPl18Sensor* getFlyskyPl18Sensor(uint16_t id, uint8_t subId)
{
  for (const FlyskyPl18Sensor* sensor = Pl18Sensor; sensor->id; sensor++) {
    if (sensor->id == id && sensor->subId == subId) {
      return sensor;
    }
  }
  return &defaultPl18Sensor;
}
                          ...
                          uint8_t instance)
{
  ...
  const FlyskyPl18Sensor* sensor = getFlyskyPl18Sensor(id, subId);
...
int32_t GetSensorValueFlySkyPl18(const FlyskyPl18Sensor* sensor,
                                 const uint8_t* data)
{
  int32_t value = 0;
  const pl18SensorData* sensorData =
      reinterpret_cast<const pl18SensorData*>(data + sensor->offset);
  if (sensor->bytes == 1)
...
void flySkyPl18Sync(int16_t delayValue)
{
  ...
  if (PL18internalModuleFwVersion >= 0x010102) {
    ...
    for (const FlyskyPl18Sensor* sensor = Pl18Sensor; sensor->id; sensor++) {
      if (sensor->id == sensorID) {
        int32_t value = GetSensorValueFlySkyPl18(sensor, ptr + 2);
        setTelemetryValue(PROTOCOL_TELEMETRY_FLYSKY_PL18, sensor->id,
                          sensor->subId, instnace, value, sensor->unit,
                          sensor->precision);
        if (sensor->id == FLYSKY_SENSOR_SYNC) flySkyPl18Sync(value);
      }
    }
  }
*/
