...
#if defined(SIMU)
extern pixel_t scratchBuf[DISPLAY_BUFFER_SIZE];
BitmapBuffer scratchBuffer(BMP_RGB565, LCD_W, LCD_H, (uint16_t *)scratchBuf);
#else
 extern uint8_t LCD_SCRATCH_FRAME_BUFFER[DISPLAY_BUFFER_SIZE * sizeof(pixel_t)];
BitmapBuffer scratchBuffer(BMP_RGB565, LCD_W, LCD_H, (uint16_t *)LCD_SCRATCH_FRAME_BUFFER);
#endif
  void newLcdRefresh();
