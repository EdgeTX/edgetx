...
struct AudioCacheEntry {
  ...
  AudioCacheEntry() : data(nullptr), size(0), lastAccessTime(0) {
    filename[0] = '\0';
  }
...
class AudioCache {
private:
  AudioCacheEntry entries[AUDIO_CACHE_MAX_ENTRIES];
  ...
  void evictLRU() {
    ...
    for (int i = 0; i < AUDIO_CACHE_MAX_ENTRIES; i++) {
      if (entries[i].data && entries[i].lastAccessTime < oldestTime) {
        oldestTime = entries[i].lastAccessTime;
        oldestIdx = i;
      }
    }
  ...
  void makeSpace(uint32_t neededSize) {
    while (totalCacheSize + neededSize > AUDIO_CACHE_MAX_SIZE && totalCacheSize > 0) {
      evictLRU();
    }
  }
...
public:
  AudioCache() : totalCacheSize(0), accessCounter(0) {}

  // Look up a file in the cache
  AudioCacheEntry* get(const char* filename) {
    for (int i = 0; i < AUDIO_CACHE_MAX_ENTRIES; i++) {
      if (entries[i].data && strcmp(entries[i].filename, filename) == 0) {
        entries[i].lastAccessTime = ++accessCounter;
        TRACE("AudioCache: Hit for %s", filename);
        return &entries[i];
      }
    }
    TRACE("AudioCache: Miss for %s", filename);
    return nullptr;
  }

  // Add a file to the cache
  bool add(const char* filename, const uint8_t* data, uint32_t size) {
    if (size > AUDIO_CACHE_MAX_SIZE / 2) {
      // Don't cache files larger than half the cache size
      return false;
    }

    // Check if already cached
    AudioCacheEntry* existing = get(filename);
    if (existing) {
    ...
    entries[slot].data = (uint8_t*)malloc(size);
    if (!entries[slot].data) {
      return false;
    }

    memcpy(entries[slot].data, data, size);
    strncpy(entries[slot].filename, filename, AUDIO_FILENAME_MAXLEN);
    entries[slot].filename[AUDIO_FILENAME_MAXLEN] = '\0';
  ...
  void getStats(uint32_t* numEntries, uint32_t* totalSize) {
    *numEntries = 0;
...
int WavContext::mixBuffer(AudioBuffer *buffer, int volume, unsigned int fade)
{
  ...
  UINT read = 0;
  AudioCacheEntry* cached = nullptr;  // Declare at function scope
  ...
  if (fragment.file[1]) {
    ...
    cached = audioCache.get(fragment.file);

    if (cached) {
      // File is cached - use cached data instead of reading from SD
      TRACE("WavContext: Using cached audio file: %s (size: %u bytes)", fragment.file, cached->size);

      // Initialize state from cached file
      // Parse RIFF header from cached data
      uint8_t* cacheData = cached->data;
      uint32_t cacheSize = cached->size;

      if (cacheSize >= RIFF_CHUNK_SIZE + 8 &&
          !memcmp(cacheData, "RIFF", 4) &&
          !memcmp(cacheData + 8, "WAVEfmt ", 8)) {

        uint32_t fmtSize = *((uint32_t *)(cacheData + 16));
        if (fmtSize < 256 && cacheSize >= RIFF_CHUNK_SIZE + 8 + fmtSize + 8) {
          state.codec = ((uint16_t *)(cacheData + 20))[0];
          state.freq = ((uint16_t *)(cacheData + 20))[2];

          // Find data chunk
          uint32_t offset = 20 + fmtSize;
          while (offset + 8 <= cacheSize && memcmp(cacheData + offset, "data", 4) != 0) {
            uint32_t chunkSize = *((uint32_t *)(cacheData + offset + 4));
            offset += 8 + chunkSize;
          }

          if (offset + 8 <= cacheSize) {
            state.size = *((uint32_t *)(cacheData + offset + 4));
            state.cacheOffset = offset + 8;  // Position after "data" header
      ...
      // Clear the filename marker AFTER we're done using it
      fragment.file[1] = 0;
    } else {
      // File not cached - read from SD card and cache it
      result = f_open(&state.file, fragment.file, FA_OPEN_EXISTING | FA_READ);
      state.usingCache = false;

      if (result == FR_OK) {
        // Get file size for caching
        uint32_t fileSize = f_size(&state.file);
        ...
        if (result == FR_OK && read == RIFF_CHUNK_SIZE+8 && !memcmp(wavBuffer, "RIFF", 4) && !memcmp(wavBuffer+8, "WAVEfmt ", 8)) {
          ...
          if (result == FR_OK && read == size+8) {
            ...
            // Attempt to cache the entire file
            if (result == FR_OK && fileSize <= AUDIO_CACHE_MAX_SIZE / 2) {
              // Save the current file position (should be at the start of audio data)
              uint32_t savedPosition = f_tell(&state.file);
              ...
              if (tempBuffer) {
                ...
                UINT bytesRead = 0;
                FRESULT cacheResult = f_read(&state.file, tempBuffer, fileSize, &bytesRead);
                if (cacheResult == FR_OK && bytesRead == fileSize) {
                  // Add to cache
                  audioCache.add(fragment.file, tempBuffer, fileSize);
                }
                free(tempBuffer);

                // Restore the file position to where we were (start of audio data)
                f_lseek(&state.file, savedPosition);
              }
            }
          }
          else {
  ...
  if (result == FR_OK) {
    ...
    if (state.usingCache && state.cachedEntry) {
      // Read from cache using the stored cache entry pointer
      AudioCacheEntry* cacheEntry = state.cachedEntry;
      uint32_t bytesToRead = min<uint32_t>(state.readSize, state.size);
      uint32_t availableInCache = cacheEntry->size - state.cacheOffset;

      if (bytesToRead <= availableInCache) {
        memcpy(wavBuffer, state.cachedData + state.cacheOffset, bytesToRead);
        read = bytesToRead;
        state.cacheOffset += bytesToRead;
      } else {
        // Not enough data in cache
        read = 0;
        result = FR_DENIED;
      }
    } else if (state.usingCache && !state.cachedEntry) {
      // ERROR: usingCache is true but we don't have cache entry pointer
      result = FR_DENIED;
    } else {
      // Read from file
...
void AudioQueue::stopSD()
{
  ...
  audioCache.clear();  // Clear cache when SD card is removed
}
