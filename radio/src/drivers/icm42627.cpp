...
static int icm42627Init(etx_i2c_bus_t bus, uint16_t addr)
{
  s_i2c_bus  = bus;
  s_i2c_addr = addr;
  ...
  if (data != 0x20) {
    TRACE("ICM426xx ERROR: this code only works on ICM42627");
    return -1;
  }
...
static int icm42627Read(etx_imu_data_t* data)
{
  uint8_t buf[6];
