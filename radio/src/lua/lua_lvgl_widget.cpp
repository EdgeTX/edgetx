...
void LvglWidgetSetting::build(lua_State *L)
{
  ...
  label = lv_label_create(window->getLvObj());
...
#if defined(HARDWARE_TOUCH)
    if (showPrev) {
      prevBtn = new IconButton(this, ICON_BTN_PREV, LCD_W - EdgeTxStyles::MENU_HEADER_HEIGHT * 3, PAD_MEDIUM, [=]() {
        prevAction();
        return 0;
      });
    }
    if (showNext) {
      nextBtn = new IconButton(this, ICON_BTN_NEXT, LCD_W - EdgeTxStyles::MENU_HEADER_HEIGHT * 2, PAD_MEDIUM, [=]() {
        nextAction();
        return 0;
      });
    }
