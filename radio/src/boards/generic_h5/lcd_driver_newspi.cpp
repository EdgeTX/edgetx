...
void lcdWriteCommand(uint8_t byte)
{
  ...
  stm32_spi_transfer_byte(&lcdSpi, byte);
...
{
  lcdSpi.SPIx = LCD_SPI;
  lcdSpi.CS = LCD_NCS_GPIO;
  lcdSpi.MISO = GPIO_UNDEF;
  lcdSpi.MOSI = LCD_MOSI_GPIO;
  lcdSpi.SCK = LCD_CLK_GPIO;
  ...
  stm32_spi_set_max_baudrate(&lcdSpi, 1000000);
...
void lcdRefresh(bool wait)
{
  ...
  for (uint8_t y=0; y < 8; y++, p+=LCD_W) {
    ...
    stm32_spi_select(&lcdSpi);
    LCD_A0_HIGH();
    stm32_spi_transfer_bytes(&lcdSpi, p, nullptr, LCD_W);
    stm32_spi_unselect(&lcdSpi);
  }
#else
