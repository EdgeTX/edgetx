...
struct TouchState touchPanelRead()
{
  ...
  if (state & 0x80u) {
    ...
    if (pointsCount > 0 && pointsCount <= GT911_MAX_TP) {
      ...
      // Panel (and thus the GT911) is mounted upside-down, matching the 180
      // display rotation in lcd_driver.cpp. Flip the reported coordinates so
      // touches line up with what is drawn.
      for (uint8_t i = 0; i < pointsCount; i++) {
        touchData.points[i].x = LCD_W - 1 - touchData.points[i].x;
        touchData.points[i].y = LCD_H - 1 - touchData.points[i].y;
      }
#endif
