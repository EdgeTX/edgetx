...
BOOTSTRAP void SystemClock_Config(void)
{
  ...
  // HSE = 48 MHz. M=12 -> 4 MHz PFD input (matches RANGE_2_4). VCO = 4*240 =
  // 960 MHz. Outputs unchanged from the previous (out-of-range M=3/N=60) config:
  // P=2 -> 480 MHz sysclk, Q=20 -> 48 MHz (USB/SPI), R=2 -> 480 MHz.
  LL_RCC_PLL1_SetM(12);
  ...
  // HSE = 48 MHz. Previous M=1 gave a 48 MHz PFD input (3x over the 16 MHz PLL
  // input max) so PLL3 could not lock -> no LTDC pixel clock. M=12 -> 4 MHz
  // input (matches RANGE_2_4). VCO = 4*48 = 192 MHz, identical to before:
  // P=2 -> 96 MHz, Q=2 -> 96 MHz, R=18 -> ~10.67 MHz LTDC pixel clock.
  LL_RCC_PLL3_SetM(12);
  LL_RCC_PLL3_SetN(48);
  LL_RCC_PLL3_SetP(2);
  LL_RCC_PLL3_SetQ(2);
