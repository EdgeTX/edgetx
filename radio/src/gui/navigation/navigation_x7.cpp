                ...
                const CheckIncDecStops &stops)
{
  ...
  if (s_editMode > 0) {
    ...
    if (event == evt_rot_inc || event == EVT_KEY_FIRST(KEY_PLUS) ||
      ...
      while ((i_flags & (INCDEC_SOURCE|INCDEC_SWITCH)) && isValueAvailable && !isValueAvailable(newval) && newval <= i_max) {
        newval++;
      }
      ...
      while ((i_flags & (INCDEC_SOURCE|INCDEC_SWITCH)) && isValueAvailable && !isValueAvailable(newval) && newval >= i_min) {
        newval--;
      }
