...
void InputEditWindow::buildBody(Window* box, ExpoData* inputData)
{
  ...
  auto gvar =
      new SourceNumberEdit(line, -100, 100, GET_DEFAULT(inputData->weight),
                           [=](int32_t newValue) {
  ...
  gvar = new SourceNumberEdit(line, -100, 100,
                              GET_DEFAULT(inputData->offset), [=](int32_t newValue) {
...
void InputEditWindow::checkEvents()
{
  ...
  for (uint8_t i=0; i<MAX_EXPOS; i++) {
    ...
    if ((ed->srcRaw == input->srcRaw) && getSwitch(ed->swtch)) {
      if (lastActiveIdx != i) {
        lastActiveIdx = i;
        updatePreview = true;
        break;
