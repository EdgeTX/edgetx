 ...
 public:
  ...
  void refresh() override
  {
    ...
    if (g_model.limitData[index].name[0] != '\0') {
      ...
      char* s = strAppend(chanStr, getSourceString({SOURCE_TYPE_CHANNEL, 0, (uint16_t)index}));
      s = strAppend(s, "\n");
...
#endif
      lv_label_set_text(source, getSourceString({SOURCE_TYPE_CHANNEL, 0, (uint16_t)index}));
    }
    if (output->revert) {
