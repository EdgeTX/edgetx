...
class GVarEditWindow: public Page
{
  ...
  protected:
    ...
    void buildHeader(Window * window)
    {
      ...
      gVarInHeader =
          new StaticText(window,
                        {PAGE_TITLE_LEFT, PAGE_TITLE_TOP + PAGE_LINE_HEIGHT, LCD_W - PAGE_TITLE_LEFT, PAGE_LINE_HEIGHT},
    ...
    {
      Page::checkEvents();
      if (gVarInHeader && ((lastFlightMode != getFlightMode()) || (lastGVar != g_model.flightModeData[getFlightMode()].gvars[index]))) {
        char label[32];
