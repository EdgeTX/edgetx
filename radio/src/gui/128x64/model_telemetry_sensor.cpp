...
//         break;
      ...
      case SENSOR_FIELD_PARAM1:
        if (sensor->type == TELEM_TYPE_CALCULATED) {
          ...
          if (sensor->unit == UNIT_RPMS) {
            ...
            if (attr) CHECK_INCDEC_MODELVAR(event, sensor->custom.ratio, 1, MIXSRC_MAX_VALUE);
      ...
      case SENSOR_FIELD_PARAM2:
        if (sensor->type == TELEM_TYPE_CALCULATED) {
          ...
          if (attr) CHECK_INCDEC_MODELVAR(event, sensor->custom.offset, -MIXSRC_MAX_VALUE, MIXSRC_MAX_VALUE);
