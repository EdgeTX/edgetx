...
{
  uint16_t value;
  // I2C bus recovery is handled centrally by the HAL transfer layer
  if (aw9523b_read(&i2c_exp, BSP_IN_MASK, &value) >= 0) inputState = value;
}
...
void bsp_output_set(uint16_t pin)
  { aw9523b_write(&i2c_exp, pin, pin); }

void bsp_output_set(uint16_t mask, uint16_t pin)
  { aw9523b_write(&i2c_exp, mask, pin); }

void bsp_output_clear(uint16_t pin)
  { aw9523b_write(&i2c_exp, pin, 0); }
