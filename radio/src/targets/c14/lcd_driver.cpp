...
static void __attribute__((unused)) LCD_Delay(void) {
...
unsigned char LCD_ReadByteOnFallingEdge(void) { return 0; }
static void lcdWriteByte(uint8_t /*data_enable*/, uint8_t /*byte*/) {}
unsigned char LCD_ReadByte(void) { return 0; }
...
void lcdWriteCommand(uint8_t command) { lcdWriteByte(0, command); }
void lcdWriteData(uint8_t data)       { lcdWriteByte(1, data); }
...
void LCD_ST7796S_Init(void) {
  ...
  // RGB interface configuration: HS/VS/DE/CLK polarity
  lcdWriteCommand(0xB0);
  lcdWriteData(0x80);
