...
{
  return (int)ImGui::GetCursorScreenPos().y + ImGui::GetStyle().WindowPadding.y;
}

static void adjust_window_height(int h)
{
  int win_w, win_h;
  SDL_GetWindowSize(window, &win_w, &win_h);

  if (h != win_h) {
    SDL_SetWindowSize(window, win_w, h);
  }
}
...
static void redraw()
{
  ...
  if (ImGui::Begin("Main window", &show_win, flags)) {
    ...
    used_height = get_used_window_height();
  }
  ImGui::End();
  adjust_window_height(used_height);
