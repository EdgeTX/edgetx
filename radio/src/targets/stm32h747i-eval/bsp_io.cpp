...
#define SD_LDO_SEL_PIN MFXSTM32L152_GPIO_PIN_13 

static bool _bsp_i2c_initialized = false;
static bool _bsp_io_initialized = false;
static MFXSTM32L152_Object_t Io_CompObj;

static int32_t BSP_I2C1_Init()
{
  if (_bsp_i2c_initialized) return BSP_ERROR_NONE;
  
  if (i2c_init(I2C_Bus_1) >= 0) {
    _bsp_i2c_initialized = true;
...
int bsp_io_init()
{
  if (!_bsp_io_initialized) {
    ...
    io_init_structure.Pull = MFXSTM32L152_GPIO_PULLDOWN;
    io_init_structure.Mode = MFXSTM32L152_GPIO_MODE_OUTPUT_PP;

    if (MFXSTM32L152_IO_Driver.Init(&Io_CompObj, &io_init_structure) !=
        MFXSTM32L152_OK) {
      return -1;
    }
...
{
  if (low_voltage) {
    MFXSTM32L152_IO_Driver.IO_WritePin(&Io_CompObj, SD_LDO_SEL_PIN, 1U);
  } else {
    MFXSTM32L152_IO_Driver.IO_WritePin(&Io_CompObj, SD_LDO_SEL_PIN, 0U);
  }
}
...
int bsp_sd_transceiver_init()
{
  ...
  sdio_diskio_set_1_8v_callback(bsp_sd_transceiver_sel_ldo);
  return 0;
}
