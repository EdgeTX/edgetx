...
Hardware notes (vs RadioMaster TX15, on which the display/LTDC wiring is based):
 - No I2C IO expander: switches, keys and trims are wired directly to GPIO
   (see boards/hw_defs/h17.json which is the source of truth for those).
...
// DEFAULT_6POS_IDX = calib index 8 (4 sticks + flex idx 4).
#define XPOS_CALIB_DEFAULT              {0x5, 0xe, 0x17, 0x20, 0x29}
#define DEFAULT_6POS_CALIB             {5, 14, 23, 32, 41}
#define DEFAULT_6POS_IDX              8

// Flex inputs are declared as S1,S2,S3,S4,6POS for the hardware pages; the
// main view slots { BL, BC, BR, left, right, left2, right2 } map back here.
#define MAIN_VIEW_FLEX_SLOTS  { 0, 4, 3, 1, 2, 5, 6 }
