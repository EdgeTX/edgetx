...
YamlParser::parse(const char* buffer, unsigned int size)
{
    ...
    while(c < end) {
        ...
        case ps_ValEsc1:
            // Match encodings generated by Companion YAML parser
            if (*c == 'b') { scratch_buf += '\b'; state = ps_ValQuo; break; }
            if (*c == 't') { scratch_buf += '\t'; state = ps_ValQuo; break; }
            if (*c == 'n') { scratch_buf += '\n'; state = ps_ValQuo; break; }
            if (*c == 'f') { scratch_buf += '\f'; state = ps_ValQuo; break; }
            if (*c == 'r') { scratch_buf += '\r'; state = ps_ValQuo; break; }
            if (*c == 'x') {
                state = ps_ValEsc2;
                break;
            }
