...
const char * FrskyChipFirmwareUpdate::waitAnswer(uint8_t & status)
{
  ...
  for (uint8_t i = 0; i < sizeof(buffer); i++) {
    uint32_t retry = 0;
    while (true) {
      if (uart_drv->getByte(uart_ctx, &buffer[i])) {
...
const char *FrskyChipFirmwareUpdate::doFlashFirmware(
    const char *filename, ProgressHandler progressHandler)
{
  const char * result;
  FIL file;
  ...
  const size_t info_len = sizeof(FrSkyFirmwareInformation);

  if (f_read(&file, buffer, info_len, &count) != FR_OK || count != info_len) {
    f_close(&file);
    return STR_DEVICE_FILE_ERROR;
  }
...
const char *FrskyChipFirmwareUpdate::flashFirmware(
    const char *filename, ProgressHandler progressHandler, bool wait)
{
  progressHandler(getBasename(filename), STR_DEVICE_RESET, 0, 0);
