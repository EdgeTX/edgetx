  ...
  splashFileName(filename)
{
  ...
  foreach(SplashLabel *sl, findChildren<SplashLabel *>()) {
    connect(sl, &SplashLabel::clicked, this, &SplashLibraryDialog::imageClicked);
  }

  connect(ui->nextPage, &QPushButton::clicked, this, &SplashLibraryDialog::nextPageClicked);
  connect(ui->prevPage, &QPushButton::clicked, this, &SplashLibraryDialog::prevPageClicked);
}
...
void SplashLibraryDialog::getFileList()
{
  imageList.clear();
  if (g.embedSplashes()) {
    const QString imgLocn(":/images/library");
    QDir myRes(imgLocn);
    QStringList tmp = myRes.entryList();

    for (int i = 0; i < tmp.size(); i++) {
      QFileInfo fileInfo(tmp.at(i));
      imageList.append(imgLocn + "/" + fileInfo.fileName());
    }
  }
...
void SplashLibraryDialog::imageClicked(int button)
{
...
void SplashLibraryDialog::nextPageClicked() {
  page++;
...
void SplashLibraryDialog::prevPageClicked() {
  page--;
