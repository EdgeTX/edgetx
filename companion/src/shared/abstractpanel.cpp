...
void AbstractPanel::addEmptyLabel(QGridLayout * gridLayout, int col)
{
  QLabel *label = new QLabel(this);
  label->setText("");
  gridLayout->addWidget(label, 0, col, 1, 1);
...
void AbstractPanel::addHSpring(QGridLayout * gridLayout, int col, int row)
{
  QSpacerItem * spacer = new QSpacerItem(0, 0, QSizePolicy::Expanding, QSizePolicy::Minimum );
  gridLayout->addItem(spacer, row, col);
}

void AbstractPanel::addVSpring(QGridLayout * gridLayout, int col, int row)
{
  QSpacerItem * spacer = new QSpacerItem(0, 0, QSizePolicy::Minimum, QSizePolicy::Expanding );
  gridLayout->addItem(spacer, row, col);
}

void AbstractPanel::addDoubleSpring(QGridLayout * gridLayout, int col, int row)
{
  QSpacerItem * spacer = new QSpacerItem(0, 0, QSizePolicy::Expanding, QSizePolicy::Expanding );
  gridLayout->addItem(spacer, row, col);
}

bool AbstractPanel::eventFilter(QObject *object, QEvent * event)
{
...
void AbstractPanel::setFocusFilter(QWidget * w)
{
  w->installEventFilter(this);
  w->setFocusPolicy(Qt::StrongFocus);
}

void AbstractPanel::disableMouseScrolling()
{
...
void AbstractPanel::save()
{
  saveAutoWidgets();
}

void AbstractPanel::saveAutoWidgets()
{
...
void AbstractPanel::updateAutoWidgets()
{
  foreach(QWidget *wdgt, findChildren<QWidget *>()) {
    AutoWidget *autowdgt = dynamic_cast<AutoWidget *>(wdgt);
    if (autowdgt) {
      autowdgt->runPreUpdate();
...
WidgetBindings *AbstractPanel::bindings()
{
  if (!m_bindings)
...
void AbstractPanel::applyBindings()
{
  if (m_bindings)
    m_bindings->applyAll();
}
