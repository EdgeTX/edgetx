...
int Boards::getCapability(Board::Type board, Board::Capability capability)
{
  switch (capability) {
    ...
    case HasIntModuleHeartbeatGPIO:
      return getCapability(board, HasExternalModuleSupport) && (IS_TARANIS(board) && !IS_FAMILY_T12(board));
    ...
    case HasTrainerModuleSBUS:
      return (IS_TARANIS_X9LITE(board) || (IS_TARANIS_XLITE(board) && !IS_TARANIS_X9LITES(board)) ||
              ...
              getCapability(board, HasIntModuleHeartbeatGPIO));
