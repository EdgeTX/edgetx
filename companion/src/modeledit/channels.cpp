  ...
  sharedItemModels(sharedItemModels)
{
  ...
  for (int i = 0; i < chnCapability; i++) {
    ...
    connect(offset[i], &RawSourceWidget::resize, [=] () { adjustSize(); });
    ...
    connect(min[i], &RawSourceWidget::resize, [=] () { adjustSize(); });
    ...
    connect(max[i], &RawSourceWidget::resize, [=] () { adjustSize(); });
    ...
    connect(curve[i], &RawSourceWidget::resize, [=] () { adjustSize(); });
