...
bool LabelsStorageFormat::write(RadioData & radioData)
{
  ...
  if (QFile(fullSettingsPath).exists()) {
    progressSetInfoAndMsg(tr("Preserve calibration..."));
    GeneralSettings gsCur;

    if (loadRadioSettings(gsCur)) {
      statusMsg(tr("Read current radio settings"));
      ...
      statusMsg(tr("Applied current calibration to new settings"));
    } else {
      fatalMsg(tr("Error reading current radio settings from %1")
                  .arg(displaySettingsPath));
      return false;
    }
  } else {
    qDebug() << "Current radio calibration not found" << displaySettingsPath;
  }
  ...
  for (const auto& model : radioData.models) {
    ...
    if (!writeChecklist(model)) {
      fatalMsg(tr("Unable to write checklist for model: %1").arg(model.name));
      return false;
    }
...
bool LabelsStorageFormat::loadChecklist(ModelData & model)
{
  ...
  if (!loadFile(model.checklistData, fname, true)) {
    fatalMsg(tr("Cannot load %1").arg(fname));
    return false;
  }
...
bool LabelsStorageFormat::writeChecklist(const ModelData & model)
{
  ...
  if (!model.checklistData.isEmpty()) {
    ...
    if (!writeFile(model.checklistData, fname)) {
      fatalMsg(tr("Cannot write %1").arg(fname));
      return false;
    }
  }
